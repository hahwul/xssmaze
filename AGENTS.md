...
XSSMaze parses its own command line (`src/cli.cr`) and then runs Kemal with
`args: nil`, so Kemal never sees ARGV. Add new flags there, and describe them
in `Banner.help` (`src/banner.cr`) — the help screen is hand-rendered, not
generated by `OptionParser`.
...
### Console output
- `src/ui.cr`: colour detection plus the palette, mirrored from the web tokens
  in `src/assets.cr`. Anything printed to a terminal goes through `UI.paint`.
- `src/banner.cr`: startup banner, help, version, and fatal CLI errors.
- `src/request_log.cr`: replaces Kemal's request logger with one aligned,
  colourised line per request. Kemal's own `Log` output is lifted to `warn`
  in `Server.start!`, so only real problems reach the console.
