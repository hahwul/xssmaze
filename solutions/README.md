...
| [modern-bypass](modern-bypass.md) | Modern real-world XSS bypasses: multi-step state, DOM clobbering config, Vue.js CSTI tag bypass, client prototype pollution, SVG direct load, iframe srcdoc, Unicode NFKC, flawed regex whitelists, closed shadow roots, slots, postMessage regex bypass, double-decoding, JSON script injection, Alpine.js, pre-CSP meta races, AngularJS CSTI, ES6 template literals, and nested query parsing prototype pollution. |
